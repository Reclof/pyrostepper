...
#include <EEPROM.h> // für die Speicherug der Werte

//**************************************************************//
//  Name    : Feuerwerk Sequenzer                               //
//  Author  : Folker Axmann                                     //
//  Date    : 14.11.2020      15.01.2023                        //
//  Version : 1.0             1.1                               //
...
// gleichzeitg geschaktet werden sollen, so muss as skript angepasst werden
// für den Stand 2020 reiht das voll aus, ich benötige nur ein einzigen Kanal je Sequenz
//
// WICHTIG!
//
// AUFGRUND von EINSCHALTVORGÄNEGN KANN ES BEIM STARTEN DES SEQUENZERS DAZU KOMMEN, DAS EIN PAAR KANÄLE (Relais) ANZIEHEN
// 
// Sicherheit geht vor!!!!!!
...
// 5. Relais auf An
// 6. Zündkreis auf An
// 7. Ab geht die Luzi! Programschalter wieder auf Lauf
// 
//
...
void loop() 
{
  ...
  if (programmlauf==false) //programm läuft nicht, benutzer darf ins menu
  {
    ...
    {   
      delay(350);
      getTaste(tastenAnalogwert);
      //Serial.println(tastenummer);

      if (editieren==false && tastenummer==5 && fragespeichern==false)
      {
        editieren=true;
        tastenummer=0;
        editKanal();
...
/*
* Anzeige zur Laufzeit aktuaisieren (Kanal, Countdown)
*/
